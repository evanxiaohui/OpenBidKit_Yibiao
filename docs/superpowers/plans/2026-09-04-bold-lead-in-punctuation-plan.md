...
- [ ] **Step 1: Write tests** for inline bold lead-ins, standalone bold lines, `__...__`, code fences, table rows, full-line image Markdown/HTML, Markdown-heading conversion, CRLF, unclosed fences, and ordinary prose.
...
- [ ] **Step 1: Implement** a fenced-code-aware line normalizer that changes sentence-final `。`/`.` inside complete bold lead-ins to `：` when inline text follows, and removes it for standalone bold lines.
- [ ] **Step 2: Call** the normalizer from `normalizeLeafContentForSave()` so all existing save paths receive the same behavior.
